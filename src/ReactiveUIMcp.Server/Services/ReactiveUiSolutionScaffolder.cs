...
public sealed class ReactiveUiSolutionScaffolder : IReactiveUiSolutionScaffolder
{
    ...
    public GeneratedReactiveUiSolutionResult Generate(CreateReactiveUiSolutionWizardRequest request)
    {
        ...
        var parsedViews = EnsureFeatureViews(request, ParseViews(request));
        ...
        WriteFile(Path.Combine(coreProjectDir, "Services", "ServiceCollectionExtensions.cs"), CreateServiceCollectionExtensionsSource(request), createdFiles);
        WriteFile(Path.Combine(coreProjectDir, "Services", "NavigationService.cs"), CreateNavigationServiceSource(request), createdFiles);
        ...
        WriteFile(Path.Combine(coreProjectDir, "Settings", "AkavacheSetup.cs"), CreateAkavacheSetupSource(request), createdFiles);
        ...
        notes.Add("Selected application features automatically add starter views, navigation entries, and service registration hints.");
        notes.Add("ReactiveUI.Validation selection now seeds starter validation rules in generated view models.");
        notes.Add("Akavache SQLite selection now emits Akavache initialization scaffolding in the generated core project.");
    ...
    {
        var results = views.ToList();

        foreach (var endpoint in request.UiEndpoints)
        {
            AddFeatureViewIfMissing(results, endpoint, request.ApplicationFeatures, "Authentication", "Login");
            AddFeatureViewIfMissing(results, endpoint, request.ApplicationFeatures, "Settings Page", "Settings");
            AddFeatureViewIfMissing(results, endpoint, request.ApplicationFeatures, "Theming", "ThemeDesigner");
            AddFeatureViewIfMissing(results, endpoint, request.ApplicationFeatures, "Navigation", "Dashboard");
        }
        ...
        string viewName)
    {
        ...
        if (views.Any(view =>
                string.Equals(view.Endpoint, endpoint, StringComparison.OrdinalIgnoreCase) &&
    ...
    private static string CreateRootReadme(CreateReactiveUiSolutionWizardRequest request)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("Generated by ReactiveUI Create Solution.");
        builder.AppendLine();
    ...
    private static string CreateAppBootstrapSource(CreateReactiveUiSolutionWizardRequest request)
    {
        ...
        builder.AppendLine($"    public const bool UsesReactiveValidation = {string.Equals(request.ValidationMode, "ReactiveUI.Validation", StringComparison.OrdinalIgnoreCase).ToString().ToLowerInvariant()};");
        builder.AppendLine("}");
        return builder.ToString();
    }

    private static string CreateServiceCollectionExtensionsSource(CreateReactiveUiSolutionWizardRequest request)
    {
        var builder = new StringBuilder();
        ...
        builder.AppendLine("{");
        builder.AppendLine("    public static IServiceCollection AddGeneratedApplicationServices(this IServiceCollection services)");
        builder.AppendLine("    {");
        ...
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("public interface INavigationService");
        builder.AppendLine("{");
        builder.AppendLine("    string CurrentRoute { get; }");
        builder.AppendLine("    void Navigate(string route);");
        builder.AppendLine("}");
        builder.AppendLine();
        builder.AppendLine("public interface IAkavacheBootstrap");
        builder.AppendLine("{");
    ...
    private static string CreateNavigationServiceSource(CreateReactiveUiSolutionWizardRequest request)
    {
        var initialRoute = request.ApplicationFeatures.Contains("Authentication", StringComparer.OrdinalIgnoreCase)
            ? "login"
            : "dashboard";
        ...
        builder.AppendLine($"namespace {request.SolutionName}.Core.Services;");
        builder.AppendLine();
        builder.AppendLine("public sealed class NavigationService : INavigationService");
        builder.AppendLine("{");
        builder.AppendLine($"    public string CurrentRoute {{ get; private set; }} = \"{initialRoute}\";");
        builder.AppendLine();
        builder.AppendLine("    public void Navigate(string route)");
        builder.AppendLine("    {");
        builder.AppendLine("        if (!string.IsNullOrWhiteSpace(route))");
        builder.AppendLine("        {");
        builder.AppendLine("            CurrentRoute = route; ");
        builder.AppendLine("        }");
        builder.AppendLine("    }");
        builder.AppendLine("}");
        return builder.ToString();
    ...
    private static string CreateAkavacheSetupSource(CreateReactiveUiSolutionWizardRequest request)
    {
        var builder = new StringBuilder();
        builder.AppendLine("using System;");
        if (string.Equals(request.SettingsStore, "Akavache SQLite", StringComparison.OrdinalIgnoreCase))
        {
            builder.AppendLine("using Akavache;");
        }
        builder.AppendLine();
        builder.AppendLine($"namespace {request.SolutionName}.Core.Settings;");
        builder.AppendLine();
        builder.AppendLine("public sealed class AkavacheBootstrap : Services.IAkavacheBootstrap");
        builder.AppendLine("{");
        builder.AppendLine("    public void Initialize()");
        builder.AppendLine("    {");
        if (string.Equals(request.SettingsStore, "Akavache SQLite", StringComparison.OrdinalIgnoreCase))
        {
            builder.AppendLine($"        Registrations.Start(\"{request.SolutionName}\");");
            builder.AppendLine("        // Add serializer and SQLite provider configuration here when wiring the generated app.");
        }
        else
        {
            builder.AppendLine("        // Akavache SQLite was not selected for this generated solution.");
        }
        builder.AppendLine("    }");
        builder.AppendLine("}");
        return builder.ToString();
    ...
    private static string CreateViewModelSource(ReactiveUiViewScaffold view, CreateReactiveUiSolutionWizardRequest request)
    {
        ...
        if (string.Equals(request.ValidationMode, "ReactiveUI.Validation", StringComparison.OrdinalIgnoreCase))
        {
            builder.AppendLine("using ReactiveUI.Validation.Extensions;");
            builder.AppendLine("using ReactiveUI.Validation.Helpers;\n");
        }
        builder.AppendLine();
        ...
        if (string.Equals(request.ValidationMode, "ReactiveUI.Validation", StringComparison.OrdinalIgnoreCase))
        {
            builder.AppendLine("    // Starter ReactiveUI.Validation rule registration placeholder.");
            builder.AppendLine("    // Example: this.ValidationRule(vm => vm.Title, title => !string.IsNullOrWhiteSpace(title), \"Title is required.\");");
        }
        else
        {
            builder.AppendLine("    // Add validation rules here if needed.");
        }

        if (request.ApplicationFeatures.Contains("Authentication", StringComparer.OrdinalIgnoreCase) && string.Equals(view.ViewName, "Login", StringComparison.OrdinalIgnoreCase))
        {
            builder.AppendLine("    // This view model was generated because Authentication was selected.");
        }

        if (request.ApplicationFeatures.Contains("Settings Page", StringComparer.OrdinalIgnoreCase) && string.Equals(view.ViewName, "Settings", StringComparison.OrdinalIgnoreCase))
        {
            builder.AppendLine("    // This view model was generated because Settings Page was selected.");
        }

        if (request.ApplicationFeatures.Contains("Theming", StringComparer.OrdinalIgnoreCase) && string.Equals(view.ViewName, "ThemeDesigner", StringComparison.OrdinalIgnoreCase))
        {
    ...
    private static string CreateWpfAppCodeBehind(string endpointProjectName) =>
        ...
        $"using {endpointProjectName.Split('.')[0]}.Core.Services;\n\n" +
        $"namespace {endpointProjectName};\n\n" +
        "public partial class App : Application\n" +
        "{\n" +
        "    public ServiceProvider Services { get; } = new ServiceCollection()\n" +
        "        .AddGeneratedApplicationServices()\n" +
    ...
    private static string CreateMauiProgramSource(string endpointProjectName, CreateReactiveUiSolutionWizardRequest request, IReadOnlyList<ReactiveUiViewScaffold> endpointViews)
    {
        ...
        return "using Microsoft.Extensions.DependencyInjection;\n" +
               ...
               $"using {request.SolutionName}.Core.Services;\n\n" +
               $"namespace {endpointProjectName};\n\n" +
